...
| `create_ticket` (problem path only) | Linear `list_teams` + `save_issue` (create) | Linear ticket conventions (the `linear-tickets` project rule — `config/opencode/project-rules/linear-tickets.md`) | ticket created; id stored in run-state | too vague to draft AC; team unresolved; create fail; draft not approved (non-autonomous) |
| `intake` | `get_issue` + `list_issue_statuses` | Linear ticket conventions (the `linear-tickets` project rule — `config/opencode/project-rules/linear-tickets.md`) | AC checklist; not blocked-by; not terminal; not oversized | intake gate fails |
