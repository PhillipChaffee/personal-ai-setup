...
#
# The OpenCode phase (#38's `I`) and the --without-opencode phase (`G`) died
# with the OpenCode unit itself: coding agents are the brain's, under herdr,
# and the catalog has no opencode unit for this installer to install or skip.
#
# Every assertion carries its id from the spec (A1..A17, B1..B4, C1, D1..D6,
# E1..E2, F1..F6, H1..H4) so a failure names the thing the installer did not
# do, rather than the line that happened to notice. The ids that are this
# file's own: E1b, because E1 as written cannot fail the way its negative
# control claims (see phase E); D-deny, which extends the deny-PATH invariant
# over the check-goose step; A9, the no-flag run's zero-skip guard; A14a is
# the seam's SHAPE claim, and #37 is what made it expressible as a diff of
# source.
#
# A DIFFERENTIAL THAT CAN SKIP IS A DIFFERENTIAL THAT CAN STOP ASSERTING (#111).
# A14b read a pre-carve blob out of git history by pinned sha and diffed the
# tree it installs against the working tree's. The pivot (#142) removed the
# opencode unit -- a deliberate behaviour change, not a regression -- so no
# pinned pre-pivot baseline can still match the tree by construction, and the
# differential died with the unit. A14b0's split-skip machinery and the
# allowlist below stay: test-deploy-vps.sh's V0/V1 differential still needs the
# shape, and A17 keeps asserting install-test.yml's fetch-depth: 0 for V1.
# refs/tags/mac-pre-carve is now inert history, kept as history is kept.
#
...
directory, in a throwaway $HOME, with no network and fixture keys.
  ...
  --only providers   phase A + check-providers.sh, and the phase E default-
                     endpoint proof
  --only select      phase A plus F+H: the flag surface (unknown ids,
                     contradictions, --only vs --with) and --dry-run
  (no flag)          all of it
...
# every skip indistinguishable from a pass to CI: an assertion whose input
# became unreachable -- as A14b's pinned baseline eventually did -- could stop
# asserting anything with no check going red.
#
...
# and check-connectors.sh:2129 (no goose CLI) are all "this box does not have
# that", which is information, not a failure. A blanket rule would either delete
# those or -- far likelier -- get worked around by an author who stops calling
# skip() and prints a note instead, which is strictly worse than a counted skip.
#
...
#          Every other way of losing a baseline is a bad(), not a skip. The list
#          is empty today: A14b died with the OpenCode unit, and no remaining
#          assertion in this file has an honest skip.
#          test-deploy-vps.sh carries the same mechanism for V0/V1.
SKIPPABLE=""
skipped() {
  # skipped <id> <message>
  case " $SKIPPABLE " in
...
# Mac without coreutils is fine.
OPTIONAL_TOOLS="timeout gtimeout"
...
#
# A LIST IN A VARIABLE, not a bare `unset` argv, so the `unset` consumes the
# same string an assertion could read; a lint that re-parsed this source text
# for a name list would be one more thing to get wrong.
...
# A1 — THE GOLDEN. Hand-written, in order, 14 lines. See the header for why it
# is typed out and not computed. Four formulae (4 probes + 4 installs), two
# casks (2 + 2), one `list --pinned` and one `pin` = 14. Not 15.
#
# RE-TYPED for the carve, and the reorder is the point rather than an accident:
# the install is unit functions called in dependency order, so brew sees each
# unit's packages together instead of seeing every formula, then every cask,
# then the pin. toolchain (uv node jq + the tailscale cask), then goose
# (formula, cask, pin). RE-TYPED AGAIN for the pivot: the opencode unit left
# the catalog (#142) and took its two lines with it. Typed out from the shapes
# the units emit, never derived from $FORMULAE_*, which would compare the code
# to itself.
...
if diff -u "$WORK/golden-a.txt" "$A_BREW" >"$WORK/golden-a.diff" 2>&1; then
  ok "A1: brew-a.log is byte-identical to the 14-line hand-written golden"
else
...
if leg brew || leg routing; then
  ...
  # B1 — seven probes, zero mutations. Also hand-written, and re-typed for the
...
EOF
  ...
  if diff -u "$WORK/golden-b.txt" "$B_BREW" >"$WORK/golden-b.diff" 2>&1 && [ "$B_MUTATIONS" -eq 0 ]; then
    ok "B1: the re-run is exactly the 7 probes, with 0 install/pin lines"
  else
  ...
  [ "$B_FORMULA_SKIPS" -eq 4 ] && [ "$B_CASK_SKIPS" -eq 2 ] && [ "$B_PINNED" -eq 1 ] &&
    ok "B2: 4 formula skips, 2 cask skips, 1 'already pinned'" ||
    bad "B2: skip lines drifted (formula=$B_FORMULA_SKIPS want 4, cask=$B_CASK_SKIPS want 2, pinned=$B_PINNED want 1)"
...
if leg select; then
  ...
  F2_RC="$(run_bootstrap_flags f2 "$WORK/home-f2" --with base-goose --without base-goose)"
  [ "$F2_RC" = "2" ] && grep -qF "base-goose is both requested and excluded" "$WORK/out/f2.log" &&
    ok "F2: --with X --without X exits 2 naming X" || {
    bad "F2: --with base-goose --without base-goose exited $F2_RC"
...
if leg select; then
  ...
  # `owns_of`'s arm for one unit could be rewritten to print another unit's
...
# ---- 8. phase D — the checks the installer tells you to run -----------------
...
# ---- 9. phase E — the defaults, with zero packets ---------------------------
if leg providers; then
  echo
...
# ---- 10. structural — the seam itself ----------------------------------------
if leg routing; then
  echo
  echo "== structural: the seam, the gate, the interlock =="
  ...
  #   (A14b, the pre-carve output differential, died with the OpenCode unit:
  #   no pinned pre-pivot baseline can match a tree that deliberately installs
  #   one unit fewer -- see the header.)
  #
...
' "$WORK/patched-code.txt" "$WORK/seam-block.txt" "$WORK/unseamed.txt" || A14_CUT=1
  ...
  #
  # BOTH JOBS, and an exact count. base-install's checkout no longer feeds a
  # differential of its own (A14b died with the OpenCode unit), but it is the
  # same fetch-depth contract and the job keeps it; the deploy-vps job's feeds
  # test-deploy-vps.sh's V0/V1/V1b, which has the same dependency for the same
  # reason (#110). A third checkout appearing here is a
