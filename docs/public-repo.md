...
- **Secret values of any kind**: `OPENCODE_ZEN_API_KEY`, `TOGETHER_API_KEY`,
  `GOOSE_SERVER__SECRET_KEY`, `TAVILY_API_KEY`, Tailscale auth keys.
...
**4. Green CI.** The latest run of `secret-scan.yml` on the default branch must be
passing:
...
**6. Post-flip.** In GitHub → Settings → Code security, enable **secret scanning** and
